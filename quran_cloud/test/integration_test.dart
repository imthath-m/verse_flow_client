...
void main() {
  group('Integration Tests', () {
    ...
    test('should fetch specific surah with ayahs', () async {
      ...
      expect(surah.name, isNotEmpty); // API returns with diacritics
      expect(surah.hasAyahs, true);
      expect(surah.ayahs, isNotEmpty);
      ...
      expect(firstAyah.text, anyOf(contains('بِسْمِ'), contains('بِسۡمِ')));
    }, timeout: Timeout(Duration(minutes: 2)));
    ...
    test('should fetch translation editions', () async {
      ...
      final englishEditions = editions.where((e) => e.language == 'en').toList();
    ...
    test('should fetch Arabic recitations', () async {
      ...
      final arabicRecitations = recitations.where((r) => r.language == 'ar').toList();
      expect(arabicRecitations.isNotEmpty, true);
      
      // Check for popular reciters
      final alafasy = recitations.where((r) => r.identifier == 'ar.alafasy').toList();
    ...
    test('should handle invalid surah number', () async {
      expect(
        () => quranService.getSurah(0),
        throwsArgumentError,
      );
      
      expect(
        () => quranService.getSurah(115),
        throwsArgumentError,
      );
    });
