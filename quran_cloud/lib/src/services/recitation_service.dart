...
class RecitationService {
  ...
  RecitationService({QuranApiClient? client}) : _client = client ?? QuranApiClient();
  
  /// Get all available audio recitation editions
  Future<List<Edition>> getAvailableRecitations() async {
    try {
      final response = await _client.get(ApiEndpoints.audioEditions());
  ...
  Future<List<Edition>> getRecitationsForLanguage(String languageCode) async {
    try {
      final response = await _client.get(ApiEndpoints.audioEditions(language: languageCode));
      final data = response.data as Map<String, dynamic>;
      
      if (data['code'] != 200) {
        throw QuranApiException(
          data['message'] ?? 'Failed to fetch recitations for language $languageCode',
          statusCode: data['code'],
          responseData: data,
        );
      }
  ...
  List<String> getAudioUrlsForRange(String editionId, int surahNumber, int startAyah, int endAyah) {
    final urls = <String>[];
  ...
  List<String> getAudioUrlsForSurah(String editionId, int surahNumber, int surahAyahs) {
