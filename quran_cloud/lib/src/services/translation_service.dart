...
class TranslationService {
  ...
  TranslationService({QuranApiClient? client})
  ...
  Future<List<Language>> getAvailableLanguages() async {
    try {
      ...
      throw QuranApiException('Failed to fetch languages', originalError: e);
    }
  }

  /// Get translation editions
  Future<List<TextEdition>> getTranslationEditions() async {
    try {
      ...
      return editionsData.map((json) => TextEdition.fromJson(json)).toList();
    } on DioException catch (e) {
      if (e.error is QuranCloudException) {
        rethrow;
      }
  ...
  Future<List<TextEdition>> getEditionsForLanguage(String languageCode) async {
    try {
  ...
  Future<List<Ayah>> getTranslationForSurah(
    String editionId,
    int surahNumber,
  ) async {
    try {
      final response = await _client.get(
        ApiEndpoints.surahWithEdition(surahNumber, editionId),
      );
      final data = response.data as Map<String, dynamic>;
  ...
  Future<List<Ayah>> getTranslationForRange(
    String editionId,
    Location start,
    Location end,
  ) async {
    ...
    if (start.surahNumber > end.surahNumber) {
      throw ArgumentError(
        'Start surah number cannot be greater than end surah number',
      );
    }
    ...
    if (start.surahNumber == end.surahNumber) {
      final surahAyahs = await getTranslationForSurah(
        editionId,
        start.surahNumber,
      );
      return surahAyahs
          .where(
            (ayah) =>
                ayah.numberInSurah >= start.ayahNumber &&
                ayah.numberInSurah <= end.ayahNumber,
          )
          .toList();
    } else {
      // Get ayahs from multiple surahs
      for (
        int surahNum = start.surahNumber;
        surahNum <= end.surahNumber;
        surahNum++
      ) {
        ...
        final endAyah = surahNum == end.surahNumber
            ? end.ayahNumber
            : surahAyahs.length;
  ...
  Future<Ayah> getTranslationForAyah(
    String editionId,
    Location location,
  ) async {
    if (!location.isValid) {
    ...
    try {
      final response = await _client.get(
        ApiEndpoints.ayahWithEdition(location.toString(), editionId),
      );
      final data = response.data as Map<String, dynamic>;
  ...
  Future<Ayah> getTranslationForAyahByNumbers(
    String editionId,
    int surahNumber,
    int ayahNumber,
  ) async {
    return getTranslationForAyah(
      editionId,
      Location(surahNumber: surahNumber, ayahNumber: ayahNumber),
    );
  }

  /// Get best translation edition for a language
  Future<TextEdition?> getBestTranslationForLanguage(
    String languageCode,
  ) async {
