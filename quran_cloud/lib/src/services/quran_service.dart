...
class QuranService {
  ...
  Future<Surah> getSurah(int number) async {
    if (number < 1 || number > 114) {
      throw ArgumentError('Invalid surah number: $number. Must be between 1 and 114.');
    }
  ...
  Future<Ayah> getAyah(Location location) async {
    ...
    try {
      final response = await _client.get(ApiEndpoints.ayah(location.toString()));
      final data = response.data as Map<String, dynamic>;
  ...
  Future<List<Ayah>> search(String query, {String? edition, int? page, int? size}) async {
