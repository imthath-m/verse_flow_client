...
class MetadataService {
  ...
  MetadataService({QuranApiClient? client})
  ...
  Future<List<Edition>> getAllEditions() async {
    try {
      ...
      throw QuranApiException('Failed to fetch editions', originalError: e);
    }
  }
  ...
  Future<List<Edition>> getEditionsByLanguage(String languageCode) async {
    final allEditions = await getAllEditions();
    return allEditions
        .where((edition) => edition.language == languageCode)
        .toList();
  }
  ...
  Future<List<Edition>> getEditionsByDirection(String direction) async {
    final allEditions = await getAllEditions();
    return allEditions
        .where((edition) => edition.direction == direction)
        .toList();
  }
  ...
  Future<List<String>> getAvailableTypes() async {
    final allEditions = await getAllEditions();
    final types = allEditions
        .map((edition) => edition.type)
  ...
  Future<Map<String, dynamic>> getEditionStatistics() async {
    ...
    for (final edition in allEditions) {
      ...
      if (edition.direction != null) {
        directions[edition.direction!] =
            (directions[edition.direction!] ?? 0) + 1;
      }
    }
  ...
  Future<Edition?> findEditionByIdentifier(String identifier) async {
    final allEditions = await getAllEditions();
    try {
      return allEditions.firstWhere(
        (edition) => edition.identifier == identifier,
      );
    } catch (e) {
      return null;
    }
